...
- `.github/workflows/build.yml`：在 `push` 和 `pull_request` 时自动还原、Debug 构建、Release 构建。非 PR 运行还会生成 x64 目录版发布包并上传 artifact。
- `.github/workflows/release.yml`：手动触发的发布工作流，可以在 GitHub Actions 页面选择版本号、运行时和发布模式。
